    ...
    Assertion.addMethod('changeTokenBalances', function (token, accounts, balanceChanges, errorMargin) {
        ...
        const derivedPromise = this.callPromise.then(async () => {
            ...
            if (ethers_1.BigNumber.from(errorMargin).eq(0)) {
                this.assert(actualChanges.every((change, ind) => change.lte(ethers_1.BigNumber.from(balanceChanges[ind]).add(errorMargin)) &&
                    change.gte(ethers_1.BigNumber.from(balanceChanges[ind]).sub(errorMargin))), `Expected ${accountAddresses} to change balance by ${balanceChanges} wei, ` +
                ...
                actualChanges.forEach((change, ind) => {
                    const low = ethers_1.BigNumber.from(balanceChanges[ind]).sub(errorMargin);
                    const high = ethers_1.BigNumber.from(balanceChanges[ind]).add(errorMargin);
