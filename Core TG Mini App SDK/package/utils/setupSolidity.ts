...
export default async function setupSolidity(targetDir: string) {
  ...
  const solidityTemplatePath = path.join(
    dirname(fileURLToPath(import.meta.url)),
    "../fileTemplates/template.sol",
  );
  ...
  const hardhatConfigPath = path.join(
    dirname(fileURLToPath(import.meta.url)),
    "../fileTemplates/hardhatConfig.cts",
  );
  ...
  try {
    ...
    packageJson.scripts = {
      ...
      "deploy-contracts":
        "npx hardhat run ./scripts/deploy.cjs --network core_testnet",
